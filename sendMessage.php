...
//常量，指定本页的内容
define('THISPAGENAME', 'sendMessage');
//引入公共文件
...
if($_GET['action'] == 'send'){
    ...
    if(!!$aResult){//判定数据库是否有选择的数据，若有再进行数据更改
        ...
        if(funcAffectRows() == 1){
            funcCloseMysql();
            funcAlertClose('信息发送成功');
        } 
        else{
            funcCloseMysql();
            funcAlertReturn('信息发送失败');
        }
        exit();
    }
    else{
...
if(isset($_GET['id'])){
    ...
    if(!!$aResult2){
        $aWebPageData = array();
        $aWebPageData['toUser'] = $aResult2['tg_username'];
        ...
        <form method="post" action="?action=send">
            ...
            <dl>
                ...
                <dd>验 证 码：<input type="text" name="captcha" class="text captcha" />
                    ...
                    <input type="submit" class="submit" value="发信息" />
                </dd>
            </dl>
        </form>
    </div>
